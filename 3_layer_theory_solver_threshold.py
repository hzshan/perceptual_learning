...
Differs from the standard solver script in that it assumes a non-zero threshold for all neurons in the network.
"""
threshold=0.005
...
parser.add('sig_s', 0.5) # default value 0.2
...
fi_mat = np.zeros((30, len(probe_angles)))
...
for i in trange(len(sig_w_array)):
    ...
    for j in range(len(probe_angles)):
        ...
        w_effs = r_utils.get_effective_weights(net, stimuli.x0, threshold=threshold, full_mat=True)[0]
        W1, W2, W3 = w_effs
        F = W3 @ W2 @ W1
        noise = F @ F.T
        u, s, v = np.linalg.svd(noise)
        fi_mat[i, j] = float(stimuli.x1.numpy() @ F.T @ np.linalg.inv(noise + s[0] * 1e-6 * np.eye(noise.shape[0])) @ F @ stimuli.x1.numpy().T)
...
file_name = 'Saved Results/theory_3L_sigs0P6'
