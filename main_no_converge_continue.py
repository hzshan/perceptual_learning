...
"""
Forked from main.py on Aug 19, 2020. This script does not have convergence detection, so it will keep running until ``n_learn`` trials are finished.

Forked from main_no_converge on Sept 11 2020. Continues gradient descent from saved results.
"""

parser = train_utils.Args('1D Gabor')
parser.add('nonlinearity', 'relu')
parser.add('CP_NAME', 'checkpoint_dir')
...
mgr = train_utils.Manager(args, device=gpu)
trainer = train_utils.Trainer(args); stimuli = train_utils.GaborStimuli(args, device=gpu)
...
cp_dir = os.getcwd() + f'/Checkpoints/{args.CP_NAME}'
result_name = cp_dir + f'/{mgr.id}.results'
saved_results = pickle.load(open(result_name, 'rb'))
delw1 = utils.recover_matrix(saved_results['delW'][-1]['layer_0'])
delw2 = utils.recover_matrix(saved_results['delW'][-1]['layer_1'])
delw3 = utils.recover_matrix(saved_results['delW'][-1]['layer_2'])
net.L0.weight.data += torch.from_numpy(delw1).to(gpu)
net.L1.weight.data += torch.from_numpy(delw2).to(gpu)
net.L2.weight.data += torch.from_numpy(delw3).to(gpu)
mgr.write('Checkpoint loaded.')
